"use client";

import Link from "next/link";
import { createClient } from "@/gateways/supabase/client";
import { Button } from "@/design-system/components/button";
import { Logo } from "@/design-system/components/logo";
import { Divider } from "@/design-system/components/divider";
import { Input } from "@/design-system/components/input";

export default function LoginPage() {
  async function handleGoogleLogin() {
    const supabase = createClient();

    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/login/callback`,
      },
    });
  }

  return (
    <div className="flex min-h-screen">
      {/* Left side - Indigo branding (hidden on mobile) */}
      <div className="relative hidden w-[45%] flex-col items-center justify-center overflow-hidden bg-surface-dark lg:flex">
        {/* Subtle radial gradient */}
        <div
          className="absolute inset-0"
          style={{
            background: "radial-gradient(ellipse at center, rgba(55, 48, 163, 0.4) 0%, transparent 70%)",
          }}
          aria-hidden="true"
        />

        <div className="relative flex flex-col items-center gap-8 px-12">
          <Logo variant="mark" size="lg" />

          <h2 className="text-center text-2xl font-bold text-white">
            Transforme avaliações com{" "}
            <span className="text-brand-200">inteligência artificial</span>
          </h2>

          {/* Feature list */}
          <ul className="flex w-full max-w-sm flex-col gap-3">
            {[
 
            ].map((feature) => (
              <li key={feature} className="flex items-start gap-3">
                <svg
                  className="mt-0.5 h-5 w-5 shrink-0 text-brand-300"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={2}
                  stroke="currentColor"
                  aria-hidden="true"
                >
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                </svg>
                <span className="text-sm text-brand-200">{feature}</span>
              </li>
            ))}
          </ul>

          <p className="max-w-xs text-center text-sm text-brand-300">
            Inteligencia Artificial para Educação Inclusiva.
          </p>
        </div>
      </div>

      {/* Right side - Login form */}
      <div className="flex flex-1 items-center justify-center bg-canvas px-6 py-12">
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-8">
            {/* Logo (visible on mobile) */}
            <div className="flex flex-col gap-6">
              <Logo size="md" variant="full" />
              <div className="flex flex-col gap-2">
                <h1 className="font-display text-3xl font-bold tracking-tight text-text-primary">
                  Bem-vindo de volta
                </h1>
                <p className="text-text-secondary">
                  Entre na plataforma para acessar suas provas e adaptações.
                </p>
              </div>
            </div>

            {/* Google OAuth */}
            <button
              type="button"
              onClick={handleGoogleLogin}
              className="flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-border-strong bg-white font-semibold text-text-primary shadow-xs transition-all duration-200 hover:shadow-soft hover:border-border-default cursor-pointer"
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Entrar com Google
            </button>

            <Divider label="ou" />

            {/* Email/password (coming soon) */}
            <div className="flex flex-col gap-4 opacity-60">
              <Input label="Email" type="email" placeholder="seu@email.com" disabled hint="Em breve" />
              <Input label="Senha" type="password" placeholder="••••••••" disabled hint="Em breve" />
              <Button variant="secondary" size="lg" fullWidth disabled>
                Entrar com email
              </Button>
            </div>

            {/* Links */}
            <div className="flex flex-col items-center gap-3">
              <p className="text-sm text-text-secondary">
                Não tem conta?{" "}
                <Link href="/login" className="font-semibold text-brand-600 hover:text-brand-700">
                  Registre-se gratuitamente
                </Link>
              </p>
              <p className="text-center text-xs text-text-muted">
                Ao continuar, você concorda com os{" "}
                <Link href="#" className="underline hover:text-text-secondary">Termos de Uso</Link>
                {" "}e{" "}
                <Link href="#" className="underline hover:text-text-secondary">Política de Privacidade</Link>.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
